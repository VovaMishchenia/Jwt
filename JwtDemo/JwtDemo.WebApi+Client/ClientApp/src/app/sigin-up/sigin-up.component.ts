...
export class SiginUpComponent implements OnInit {
  ...
  register() {
    ...
    if (this.model.FullName == "" || this.model.Address == "" || this.model.Email == "" || this.model.Password == "" || this.model.Phone == "" || this.confirmPassword == "") {
      this.spinner.hide();
      this.notifier.hideAll();
          ...
          (data) => {
            ...
            if (data.statusCode === 200) {
              ...
              if (this.model.Password.length > 5) {
                this.notifier.notify("error", "Registration problem. There is registered user with the same email");
              }
              else {
                this.notifier.notify("error", "Registration problem. Your password must contains more than 5 symbols");
              }
            }
          }
        )
